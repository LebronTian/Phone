...
function do_delete(uids) {
	...
	var data = {uids: uids.join(';'), b_uid: g_b_uid}
	$.post('?_a=book&_u=api.delrecord', data, function(ret){
		console.log(ret);
...
function do_remark(uids, sp_remark) {
	...
	var data = {uids: uids.join(';'), b_uid: g_b_uid, sp_remark: sp_remark}
	$.post('?_a=book&_u=api.markrecord', data, function(ret){
		console.log(ret);
		window.location.reload();
	});
}
...
$('.csp_remarks li').click(function(){
	...
	if ($('.ceditall').hasClass('am-text-danger')||!$('.beditall').hasClass('am-btn-success')) {
		...
		window.location.href='?_a=book&_u=sp.recordlist&b_uid='+g_b_uid+'&sp_remark=' + r;
	}
});
...
$('.option_key_btn').click(function(){
	...
	//允许关键字为空，表示清空条件
	if(1 || key) {
		window.location.href='?_a=book&_u=sp.recordlist&b_uid='+g_b_uid+'&key='+key;
	}
});
...
$('.option_cat').change(function(){
	...
	if(1 || key) {
		var cat = $(this).val();
		window.location.href='?_a=book&_u=sp.recordlist&b_uid=' + cat+'&key='+key;
	}
});
