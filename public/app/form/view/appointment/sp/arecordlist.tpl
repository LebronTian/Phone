<?php
	...
	$pagination = uct_pagination($option['page'], ceil($records['count']/$option['limit']), 
			'?_a=form&_u=sp.arecordlist&f_uid='.$option['f_uid'].'&key='.$option['key']
        ...
        if(!$records['list']) {
            ...
            foreach($records['list'] as $c) {
                ...
                $html .= ' data-id="'.$c['uid'].'"><td class="table-check"><input type="checkbox" class="ccheck"></td>'.
                    ...
                    '<td>'.@$c['data'][1].'</td>'.
                    '<td>';
					if(($su_uid = checkInt(@$c['data'][2]))) {
						if($p = SuMod::get_su_profile($su_uid)) {
							$html .= $p['realname'].' <span class="am-icon-mobile"></span> '.$p['phone'];
						}
						else if($p = AccountMod::get_service_user_by_uid($su_uid)) {
							$html .= $p['name'] ? $p['name'] : $p['account'];
						}
					}
					else {
						$html .= '-';
					...
					$html .= '</td>'.
                    '<td>'.(!empty($c['order']['paid_time']) ? date('Y-m-d H:i:s', $c['order']['paid_time']) : '-').'</td>';
...
$extra_js = $static_path.'/js/sp.arecordlist.js';
?>
