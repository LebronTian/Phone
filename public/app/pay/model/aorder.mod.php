...
class AOrderMod {
	public function __construct($uid) {
		if(!($this->o = SpServiceMod::get_service_order_by_uid($uid))) {
			setLastError(ERROR_OBJ_NOT_EXIST);
		}

		//系统支付成功 注意不要重复添加事件
		Event::addHandlerOnce('AfterServiceOrderPay', array('SpServiceMod', 'onAfterServiceOrderPay'));
	}
	...
	*/
	public function PreparePayInfo() {
		...
		return array(
					'trade_no' => 'a'.$this->o['uid'],
					'total_fee'=> $this->o['paid_fee'],
					'title'    => $this->o['service']['name'],
					'detail'   => $this->o['service']['name'].'      X 1',
					'product_uid' => $this->o['service']['uid'],

					'sp_uid' => 0,
					'su_uid' => $this->o['sp_uid'],
					);
	}
	...
	*/
	public function GetOrderInfo() {
		...
		return array(
					...
					'total_fee'=> $this->o['paid_fee'],
					'title'    => $this->o['service']['name'],
					'return_url' => !empty($this->o['service_uid']) 
									? '?_a=sp&_u=index.servicedetail&uid='.$this->o['service_uid'].'&dir=' 
									.(!empty($this->o['service']['dir']) ? $this->o['service']['dir'] : '')
									: '?_a=sp&_u=index.orderdetail&uid='.$this->o['uid'],
	...
	*/
	public function SavePayInfo($params) {
		...
		return SpServiceMod::make_a_service_order($update);
	}
	...
	*/
	public function PaySucceedCallback($params) {
		...
		Dba::beginTransaction(); {
			SpServiceMod::make_a_service_order($update);
			//付款成功, 服务订单增加对应配额,把状态改为已收货, todo 通知运营管理员
			Event::handle('AfterServiceOrderPay', array(SpServiceMod::get_service_order_by_uid($this->o['uid'])));
		} Dba::commit();
		return true;
	}
