...
class COrderMod {
	public function __construct($uid) {
		uct_use_app('shop');
		uct_use_app('takeaway');
		if(!($this->o = TakeawayMod::get_takeaway_order_by_order_uid($uid))) {
		...
		//支付成功 注意不要重复添加事件
		Event::addHandlerOnce('AfterTakeawayOrderPay', array('TakeawayMod', 'onAfterTakeawayOrderPay'));
		}
	}
	...
	*/
	public function GetOrderInfo() {
		...
		return array(
					...
					'return_url' => '?_a=takeaway&_u=index.orderdetail&uid='.$this->o['uid'],
	...
	*/
	public function PaySucceedCallback($params) {
		...
		$update = array('uid' => $this->o['uid'],
						'status' => OrderMod::ORDER_WAIT_SHOP_ACCEPT, //外卖订单标记订单为等待厨师确认
						'pay_type' => $params['pay_type'],
		...
		Dba::beginTransaction(); {
			...
			//付款成功, 通知厨师接单
			Event::handle('AfterTakeawayOrderPay', array(TakeawayMod::get_takeaway_order_by_order_uid($this->o['uid'])));
		} Dba::commit();
		return true;
	}
