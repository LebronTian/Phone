...
class GOrderMod {
	public function __construct($uid) {
		uct_use_app('su');
		if(!($this->o = SuChargeMod::get_sucharge_order_by_uid($uid))) {
			setLastError(ERROR_OBJ_NOT_EXIST);
		}

		//系统支付成功 注意不要重复添加事件
		Event::addHandlerOnce('AfterSuChargeOrderPay', array('SuChargeMod', 'onAfterSuChargeOrderPay'));
	}
	...
	*/
	public function PreparePayInfo() {
		...
		return array(
					'trade_no' => 'g'.$this->o['uid'],
					'total_fee'=> $this->o['paid_fee'],
					'title'    => '充值'.($this->o['charge']['charge_price']/100).'元',
					'detail'   => '充值'.($this->o['charge']['charge_price']/100).'元      X '.$this->o['charge']['quantity'],
					'product_uid' => $this->o['charge_uid'],
	...
	*/
	public function GetOrderInfo() {
		...
		return array(
					...
					'total_fee'=> $this->o['paid_fee'],
					'title'    => '充值'.($this->o['charge']['charge_price']/100).'元',
					'return_url' => '?_a=su&_u=index.chargedetail&uid='.$this->o['uid'],
	...
	*/
	public function SavePayInfo($params) {
		...
		return SuChargeMod::make_a_sucharge_order($update);
	}
	...
	*/
	public function PaySucceedCallback($params) {
		...
		Dba::beginTransaction(); {
			SuChargeMod::make_a_sucharge_order($update);
			//付款成功
			Event::handle('AfterSuChargeOrderPay', array(SuChargeMod::get_sucharge_order_by_uid($this->o['uid'])));
		} Dba::commit();
		return true;
	}
