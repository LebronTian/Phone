...
function do_delete(uids) {
	...
	var data = {uids: uids.join(';'), r_uid: g_r_uid}
	$.post('?_a=reward&_u=api.delrewardrecord', data, function(ret){
		console.log(ret);
...
function do_remark(uids, sp_remark) {
	...
	var data = {uids: uids.join(';'), r_uid: g_r_uid, sp_remark: sp_remark}
	$.post('?_a=reward&_u=api.markrewardrecord', data, function(ret){
		console.log(ret);
		window.location.reload();
	});
}
...
$('.csp_remarks li').click(function(){
	...
	if ($('.ceditall').hasClass('am-text-danger')||!$('.beditall').hasClass('am-btn-success')) {
		...
		window.location.href='?_a=reward&_u=sp.recordlist&r_uid='+g_r_uid+'&sp_remark=' + r;
	}
});
...
$('.option_key_btn').click(function(){
	...
	//允许关键字为空，表示清空条件
	if(1 || key) {
		window.location.href='?_a=reward&_u=sp.recordlist&r_uid='+g_r_uid+'&key='+key;
	}
});
...
$('.option_cat').change(function(){
	...
	if(1 || key) {
		var cat = $(this).val();
		window.location.href='?_a=reward&_u=sp.recordlist&r_uid=' + cat+'&key='+key;
	}
});
