...
function RepeatedImgLayer(a, b, c, d) {
    ...
    this.animation = createjs.Tween.get(this, {
        loop: !0
    }).to({
        x: -d
    }, d / c * 1E3);
...
function ImageButton(a, b, c) {
    ...
    b && (b = new createjs.Bitmap(queue.getResult(b)),
    this.addChild(b),
    a.x = (b.image.width - a.image.width) / 2,
    a.y = (b.image.height - a.image.height) / 2);
    this.addChild(a);
    c && (c = this.getBounds(),
    this.regX = c.width / 2,
    this.regY = c.height / 2);
...
ImageButton.prototype.setEnabled = function(a) {
    (this.mouseEnabled = a) ? this._removeFilter("disable") : (a = new createjs.ColorMatrixFilter((new createjs.ColorMatrix).adjustBrightness(-80).adjustSaturation(-100)),
    this._addFilter(a, "disable"))
}
;
function loadFollowRes() {
    USE_NATIVE_SHARE || queue.loadManifest({
        path: BASE_RES_DIR + "img/",
        manifest: [{
            src: "follow_anim.png",
            id: "follow"
        }]
...
function setFollowParent(a) {
    USE_NATIVE_SHARE || (g_followAnim.parent.removeChild(g_followAnim),
    a.addChild(g_followAnim))
}
function setFollowAnim(a) {
    if (!USE_NATIVE_SHARE && IS_REFFER) {
        var b = g_followAnim.getBounds();
        a ? (g_followAnim.play(),
        createjs.Tween.get(g_followAnim).to({
            regX: b.width,
            regY: 0,
            visible: !0
        }).to({
            regX: 0,
            regY: b.height
        }, 500)) : createjs.Tween.get(g_followAnim).to({
            regX: b.width,
            regY: 0
        }, 500).to({
            visible: !1
...
USE_NATIVE_SOUND ? createjs.Sound.play = function(a) {
 ...
 : IS_NATIVE_ANDROID && (createjs.Sound.play = function(a, b) {
    ...
    c.play();
    void 0 != b && !0 == b && (null  != g_androidsoundtimer && (clearTimeout(g_androidsoundtimer),
    g_androidsoundtimer = null ),
    g_androidsoundtimer = setTimeout(function() {
