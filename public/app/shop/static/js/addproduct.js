...
$(document).ready(function () {
    ...
    $("#saveProduct").click(function () {
        ...
        if((Info|32)==Info){
            if($("#groupPrice").val()>0){
                showTip('err',"团购秒杀不能同时存在！",1000);
                console.log($("#groupPrice").val());
                return
            }
...
//      var else_info = $("#else_info").val();
		...
		var buy_limit = parseInt($("#buy_limit").val());
        /*上传！！！！！！！！！！！！！！！！！！*/
        var bigData = {
            "title":title,
...
//          "else_info":else_info,
        ...
        var link,text;
        if(uid){
            link = "/?_a=shop&_u=api.addproduct&uid="+uid;
            text = '保存成功'
        }else{
            link = "/?_a=shop&_u=api.addproduct";
            text = '添加成功'
        }
        console.log(bigData);
        $.post(link,bigData, function(data){
            ...
            if(data.errno==0){
                ...
                if(extra_length==0){
                    window.location.href="/?_a=shop&_u=sp.productlist&page="+page;
                }
                else{
                    $(".extra-box").each(function (index) {
                        ...
                        console.log(data);
                        $.post("?_a=shop&_u=api.add_product_extra_info",data, function (ret) {
                            console.log(ret);
                            /*判断是否结束************************/
