$(document).ready(function () {
    ...
    * */
    $(".template-save").click(function () {
        if(!($("#addtemplate-body").find(".am-field-error").length==0)){
            showTip("err","存在填写错误的地方，请正确填写",1000);
            return
        }
        var title = $("#template_title").val();
        var brief = $("#template_brief").val();
        if(title.trim()==""){
            showTip("err","请输入模板名称",1000);
            return
        }
        var valuation;
        var rule={};
        ...
        console.log(data);
        var link = "/?_a=shop&_u=api.adddelivery";
        $.post(link,data, function(data,status){
            showTip("","添加成功",1000);
            window.location.href="/?_a=shop&_u=sp.deliverytemplate"
        });
    });
    /****************************************************************/
     /* 编辑初始化 */
