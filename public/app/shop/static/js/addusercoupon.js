...
$(document).ready(function () {
    ...
    $('.save').click(function(){
        var coupon_uid = $('.option_duration').val();
        var user_id=[];
        ...
        var publish_cnt = $('#id_send_cnt').val();
        var data = {
        ...
        console.log(data);
        $.post('?_a=shop&_u=api.addusercoupon', data, function(ret){
            console.log(ret);
            ret = $.parseJSON(ret);
            if(ret.errno==0){
                showTip('ok','发放成功',1000);
                setTimeout("history.back()",1000);
            }
            else {
                if(ret.errno=403)
                {
                    showTip('err','有用户接收该优惠券数量到达上限',1500);
                }
                else
                {
                    showTip('err','发放出错',1000);
                }
                //setTimeout("history.back()",1000);
            }
        });
...
/*
 var html =
 '<li class="select2-selection__choice" title="阿斯顿">' +
 '<span class="select2-selection__choice__remove" role="presentation">×</span>阿斯顿' +
 '</li>';
 $(".select2-search").before(html)*/