...
$(document).ready(function () {
    ...
    $(".delCopy").click(function () {
		window.location.href="?_a=shop&_u=admin.addproduct&copy_uid="+$(this).attr( "data-id");
		return;
        var r = confirm("确定要复制？");
        if(r==true){
            var uid = $(this).attr( "data-id");
            var data = {uid:uid};
            $.post('?_a=shop&_u=admin.copyproduct', data, function(ret){
                ret = $.parseJSON(ret);
                console.log(ret.data);
                //window.location.reload();
                window.location.href="?_a=shop&_u=admin.addproduct&uid="+ret.data;
            });
        }
    });
    ...
    $('.pStatus').click(function(){
        ...
        $.post('?_a=shop&_u=admin.add_product', data, function(ret){
            console.log(ret);
            window.location.reload();
        });
    });
...
function do_delete(uids) {
    ...
    $.post('?_a=shop&_u=admin.delproduct', data, function(ret){
        window.location.reload()
    });
}
...
$('.option_key_btn').click(function(){
	...
	//允许关键字为空，表示清空条件
	if(1 || key) {
		window.location.href='?_a=shop&_u=admin.prodictgrouplist'+'&key='+key;
	}
});
