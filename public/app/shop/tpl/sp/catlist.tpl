...
<div class="am-g am-padding">
    ...
    <a  href="?_a=shop&_u=sp.addcat" type="button" class="am-btn am-btn-success"><span class="am-icon-plus"></span> 添加分类</a>
	</div>
	...
	if(!$cats) {
		...
		foreach($cats as $c) {
			...
			$html .= ' data-id="'.$c['uid'].'"><td class="table-check"><input type="checkbox" class="ccheck"></td><td>'.
					$c['title'].($c['title_en'] ? ' ('.$c['title_en'].')' : '').'</td><td>'.(!empty($c['parent_cat']['title']) ? $c['parent_cat']['title'] : '顶级').'</td><td><img src="'.$c['image'].'" style="max-width:100px;max-height:100px;"></td><td>'
