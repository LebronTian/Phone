...
<div class="am-g am-padding">
    ...
    <a  href="?_a=shop&_u=sp.addslide" type="button" class="am-btn am-btn-success"><span class="am-icon-plus"></span> 添加幻灯片</a>
	</div>
...
<?php
	if(!$slides) {
		...
		foreach($slides as $c) {
			if($c['slides_in'] == 0){
				$c['slides'] = '首页';
			}elseif($c['slides_in'] == 1){
				$c['slides'] = '留言墙';
			}elseif($c['slides_in'] == 2){
				$c['slides'] = 'PC首页';
			}
			$html .= '<tr';
			if($c['status']) {
			...
			$html .= ' data-id="'.$c['uid'].'"><td class="table-check"><input type="checkbox" class="ccheck"></td>'.
				'<td><img src="'.$c['image'].'" style="max-width:100px;max-height:100px;"></td><td>'
					.$c['sort'].'</td>';
			$html .='<td>'.$c['slides']. '</td>';
			$html .='<td>'.($c['status'] ? '<a data-status="'.$c['status'].'" class="cstatus am-btn am-btn-xs am-btn-danger">已禁用</a>':
			...
			$html .= '<a href="?_a=shop&_u=sp.addslide&uid='.$c['uid']
