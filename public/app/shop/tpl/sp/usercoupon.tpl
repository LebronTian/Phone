...
<div class="am-cf am-padding">
  <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">已发放优惠劵</strong> / <small>总计 <?php echo $data['count'];?> 张</small></div>
</div>
...
<?php
	if(!$data['list']) {
		...
		foreach($data['list'] as $c) {
			$html .= '<tr';
			if($c['order_uid'] > 0 || ($c['expire_time'] >0 && ($c['expire_time'] < $_SERVER['REQUEST_TIME']))) {
