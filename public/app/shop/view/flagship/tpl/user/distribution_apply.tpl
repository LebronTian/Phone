            ...
            <div class="bd spacing">
                <div class="weui_msg" style="padding-top: 0">
                    ...
                    <div class="weui_text_area">
                        ...
                        <p onclick="window.location.href='?_a=shop&_u=user.distribution_center'" class="weui_msg_desc" style="cursor: pointer">点击这里进入管理中心</p>
                    </div>
                </div>
            </div>
        <?php
        }
        else if($distribution['status']==2){
            ?>
            <div class="bd spacing">
                <div class="weui_msg" style="padding-top: 0">
    ...
    seajs.use(['weui/slide'], function (slide) {
        $(function () {
            ...
            $('.apply-btn').on('click', function () {
                $('#loadingToast').show();
                $.post('?_a=shop&_u=ajax.distribution_apply', function (ret) {
                    ret = JSON.parse(ret);
                    setTimeout(function () {
                        if(ret.errno==0){
                            slide.go('result');
