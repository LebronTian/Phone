$('#id_group').change(function(){
	var uid = $(this).val();
	if(uid != this_g_uid) {
		window.location.href='?_a=shopdist&_u=sp.distribution_user_group_set&uid=' + uid;
	}
});
...
$(document).ready(function () {
    ...
    $("#saveBiz").click(function () {
        ...
        var g_uid = $('#id_group').val();

        if(!g_uid){
            showTip("err","请选择分组",1000);
            return
        }
        ...
        console.log(bigData);
        $.post('/?_a=shop&_u=api.set_group_distribution',bigData, function(data){
            data = $.parseJSON(data);
            console.log(data);
            if(data.errno==0){
                showTip("",'添加成功',"1000");
