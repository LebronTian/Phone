function do_delete(uids) {
	...
	$.post('?_a=site&_u=api.delreply', data, function(ret){
		console.log(ret);
...
function do_pass(uids) {
	...
	$.post('?_a=site&_u=api.reviewreply', data, function(ret){
		console.log(ret);
		window.location.reload();
	});
}
...
$('.option_key_btn').click(function(){
	...
	//允许关键字为空，表示清空条件
	if(1 || key) {
		window.location.href='?_a=site&_u=sp.replylist&key='+key;
	}
});
...
function do_reply(uid) {
	var sort = $('#id_reply').val();
	if(!sort) {
		return alert('请填写顶置序号');
	}

	$.post('?_a=site&_u=api.addreply', {uid:uid, sort: sort}, function(ret){
		console.log(ret);
		ret = $.parseJSON(ret);
		if(ret.errno == 0) {
			window.location.reload();
  ...
  $('.creply').on('click', function() {
  	var uid = $(this).attr('data-id');
  	var sort = $(this).attr('data-value') || 0;
	$('#id_reply').val(sort);
      $('#my-confirm').modal({
        relatedTarget: this,
