...
<div class="am-cf am-padding">
  <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">评论管理</strong> / <small>总计 <?php echo $data['count'];?> 篇</small></div>
</div>
...
<table class="am-table am-table-striped am-table-hover table-main">
	<thead>
		<tr>
			...
			<th class="table-contact">评论文章</th>
			<th class="table-brief">评论内容</th>
			<th class="table-brief">评论时间</th>
...
<?php
	if(!$data['list']) {
		...
		foreach($data['list'] as $m) {
			...
			$html .= '<a href="?_a=su&_u=sp.fansdetail&uid='.$m['su_uid'].'">'.($m['su']['name'] ? 
						$m['su']['name'] : $m['su']['account']).'</a>';
			...
			if(!$m['article_uid']) {
				...
				$title = Dba::readOne('select title from site_articles where uid = '.$m['article_uid']);
				$html .= '</td><td><a href="?_a=site&_u=sp.addarticle&uid='.$m['article_uid'].'">'.$title.'</a></td>';
			}
			$html .= '<td>'.$m['brief'].'</td><td>'.date('Y-m-d H:i:s', $m['create_time']).'</td><td>'.'<div class="am-dropdown private_verify" data-am-dropdown>
  <button class="am-btn ';
	if($m['status']==0)
		$html.='am-btn-default';
	if($m['status']==1)
  ...
  <ul class="am-dropdown-content creview" data-uid="'.$m['uid'].'" style="min-width:40px !important; cursor:pointer; text-align:center;">
    ...
    <li class="am-btn-danger msg_unpass" data-id="'.$m['uid'].'" sp="2">拒绝</li>
			...
			if(empty($m['sort']) ) {
			$html .= ' <button class="am-btn am-btn-default am-btn-xs am-text-warning creply" data-value="0" data-id="'.$m['uid'].'"><span class="am-icon-arrow-up"></span> 顶置</button>';
			}
			else {
			$html .= ' <button class="am-btn am-btn-default am-btn-xs am-text-success creply" data-value="'.$m['sort'].'" data-id="'.$m['uid'].'"><span class="am-icon-arrow-up"></span> 已顶置</button>';
			}
...
<div class="am-modal am-modal-confirm" tabindex="-1" id="my-confirm">
  <div class="am-modal-dialog ">
    <div class="am-modal-hd">留言顶置</div>
	<hr/>
	<form class="am-form am-form-horizontal">
    	<div class="am-form-group">
			<label class="am-u-sm-4 am-form-label" for="id_reply">数字越大越靠前</label> 		
			<div class="am-u-sm-8">
...
<?php
	$extra_js = $static_path.'/js/replylist.js';
?>
