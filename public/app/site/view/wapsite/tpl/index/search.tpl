...
<div class="title_box">
	<div class="red_box">
		<div class="list_title">搜索结果</div>
		<div class="list_title_en">COMPANY PROFILE</div>
	</div>
	<div class="blue_box">
	...
	<div class="list_box">
		<ul class="article_list" data-id="<?php echo $catid; ?>" data-key="<?php echo $option['key'];?>">
		</ul>
	</div>
...
var key=$('.article_list').attr('data-key');
$(document).ready(function(){
	uid=$('.article_list').attr('data-id');
  key=$('.article_list').attr('data-key');
	ajaxList();
	setTimeout(function(){
...
function ajaxList(){
  var html='';
  var link='?_a=site&_u=ajax.article_list&key='+key+'&cat_uid='+uid+'&limit=6&page='+apage;
  $.getJSON(link,function(data){
  	var data=data.data.list;
    console.log(data);
    apage++;
    $.each(data, function () {
      html+='<li data-uid="'+this.uid+'"><a href="?_a=site&_u=index.article&cid='+this.uid+'"><div class="article_box"><img src="'+this.img+'"><div class="article_title">'+this.title+'</div></div></a></li>';
    });
