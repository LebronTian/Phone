        ...
        $("#fake-confirm").modal({
            ...
            onConfirm: function() {
                $.post("?_a=sp&_u=api.add_fake_public", function (ret) {
                    ...
                    console.log(ret);
                    if(ret.errno==0){
                        agoto('?_a=sp&_u=index.publiclist');
                    }
                    else{
    ...
    $.post('?_a=sp&_u=api.add_public', data, function(ret){
        ...
        if(ret.errno==0){
            if($(that).hasClass("over-btn")){
                ...
                showTip('',text,1000);
                setTimeout('agoto("?_a=sp&_u=index.publiclist")',1000);
            }else{
                /*下一步按钮*/
                showTip('err','提交公众号信息成功',1000);
                nextBtn();
                //获取appid
                g_uid = ret.data;
                console.log("guid",g_uid);
        ...
        else showTip('err','提交公众号信息失败',1000);
    });
});
