...
function do_delete(uids) {
	...
	$.post('?_a=sp&_u=api.delete_feedback', data, function(ret){
		console.log(ret);
...
$('.option_cat').change(function(){
	...
	var cat = $(this).val();
	window.location.href='?_a=sp&_u=index.feedbacklist&status=' + cat;
});
...
$('.option_key_btn').click(function(){
	...
	if(1 || key) {
		var cat = $(this).val();
		window.location.href='?_a=sp&_u=index.feedbacklist&status=' + cat+'&key='+key;
	}
});
