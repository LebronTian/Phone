<div class="am-cf am-padding">
  <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">附件管理</strong> / <small>共计<?php echo $attaches ? count($attaches) : 0;?>个</small></div>
</div>

<div class="am-g am-padding">
	<div class="am-fl am-cf">
    <a  href="?_a=sp&_u=index.addattach&pos=<?php echo $option['pos']?>" type="button" class="am-btn am-btn-success"><span class="am-icon-plus"></span> 添加附件</a>
	</div>
...
<table class="am-table am-table-striped am-table-hover table-main">
	<thead>
		<tr>
			...
			<th class="table-image">编号</th>
			<th class="table-image">logo</th>
	...
	if(!$attaches) {
		...
		foreach($attaches as $c) {
			...
			$html .= ' data-id="'.$c['uid'].'"><td class="table-check"><input type="checkbox" class="ccheck"></td>'.
				...
				'<td>'.($c['pos'] ? AttachesMod::get_pos($c['pos']) : '-').'</td>'.'<td>'
			...
			$html .= '<a href="?copy_uid='.$c['uid']
				.'&_a=sp&_u=index.addattach" target="_self" class="am-btn am-btn-default am-btn-xs am-text-primary"><span class="am-icon-copy"></span> 复制</a>';
			$html .= '<a href="?_a=sp&_u=index.addattach&uid='.$c['uid']
...
<?php
	$extra_js = $static_path.'/js/attachlist.js';
?>
