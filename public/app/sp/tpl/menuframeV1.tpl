...
<?php
	...
	if(!empty($GLOBALS['_UCT']['TPL'])) {
		$file = UCT_PATH.'app'.DS.$GLOBALS['_UCT']['APP'].DS.'view'.DS.$GLOBALS['_UCT']['TPL'].DS.'menuarray.php';
		if(file_exists($file) && ($ems = include $file)) {
			if(is_array($ems)) {
				array_walk($ems, function(&$v) {
					$v['is_extra'] = 1;
				});
				$menu_array = array_merge($menu_array, $ems);
			}	
		}
		#var_export($ems);
	}
	...
	foreach($menu_array as $i => $m) {
		if(empty($m['menus'])) { 
			...
			if(!SpInviteMod::check_current_sp_access(array('ctl' => $ctl, 'act' => $act))) {
				continue;
			}
			...
			$html .= '"><a class="am-cf " data-am-collapse="{target: \'#collapse-nav'.$i.'\'}"><span class="'
					...
					'<ul class="am-list am-collapse admin-sidebar-sub am-in2" id="collapse-nav'.$i.'">';
