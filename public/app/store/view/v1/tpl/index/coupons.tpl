...
<div style="text-align: center; font-size: 16px; margin-top: 50px;"><a href="?_a=store&_u=index.usercoupons"></a></div>
<section class="LeafletCon">
<?php
	if(empty($coupons['list'])) {
		echo '<div style="text-align: center; font-size: 14px; margin-bottom: 15px;">【目前还没有优惠券哟, 请留意我们的最新活动】</div>';
	}
	else {
		$html = '
...
<div class="msg_card">
    <div class="card_content';
		$available = $c['used_cnt'] < $c['publish_cnt'];
		#var_export($c);
		if(!$available) $html .= ' disabled';
		$html .= '" style="background-color:#55BD47;">
        <img class="logo" src="'.$c['img'].'">
        <div class="card_info">
            <h4 class="card_title" ><a href="?_a=store&_u=index.coupondetail&uid='.$c['uid'].'">'.$c['title'].'</a></h4>
            <p class="store">剩余: '.max(0, $c['publish_cnt'] - $c['used_cnt']).' 张</p>
			';
		if(!empty($c['store_uids'][0])) {
			$html .= '<p class="store2"> 适用店铺: '.Dba::readOne('select name from store where uid = '.$c['store_uids'][0]).'</p>';
		}
        $html .= '</div>
        <div class="deco"></div>
    </div>
    <div class="card_bottom">';
		if($available) {
	 		$html .= '
		        <a class="btn btn_primary cget" data_uid="'.$c['uid'].'">立即领取</a>';
		}
	$html .= '</div></div>';
	}
