...
<?php
	$su = AccountMod::get_service_user_by_uid($coupon['user_id']);
	echo '
<div class="msg_card">
    <div class="card_content">
		...
		echo '<hr>';
		if($coupon['used_time']) {
			echo '<p class="btn btn_primary btn_disabled" >'.date('Y-m-d H:i', $coupon['used_time']).' 已核销</p>';
		}
		else if($coupon['expire_time'] && $coupon['expire_time'] <= $_SERVER['REQUEST_TIME']) {
			echo '<p class="btn btn_primary btn_disabled" >已过期</p>';
		}
		else {
			echo '<p class="btn btn_primary" id="id_writeoff" data-id="'.$coupon['uid'].'">点击核销</p>';
		}
	echo '</div>
</div>
		';
...
<script>
$('#id_writeoff').click(function(){
	if(!confirm('确定要核销吗?')) return;
	var uid = $(this).attr('data-id');
	$.get('?_a=store&_u=ajax.writeoff_coupon&uid=' + uid, function(ret){
		console.log(ret);
		ret = $.parseJSON(ret);
		if(ret && ret.data) {
			alert('核销成功!');
