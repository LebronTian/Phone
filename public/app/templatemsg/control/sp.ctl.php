	...
	*/
	public function get_menu_array()
	{
		...
		*/
		...
		$even_arr = Template_Msg_WxPlugMod::get_even_arr();
		$menus_arr_key =array();
		foreach ($even_arr as $ek => $e)
			foreach ($e as $k => $menus)
				...
				$menus_arr[$ek][] = array('name'      => $menus['title'],
					...
					'link'      => '?_a=templatemsg&_u=sp.templatelist&even=' . (1000 * $ek + $k),
		...
		return array(
			array('name'      => '首页',
			      'icon'      => 'am-icon-home',
			      'link'      => '?_a=templatemsg&_u=sp',
			      'activeurl' => 'sp.index'),
			array('name'      => '发送记录',
			      'icon'      => 'am-icon-list',
			      'link'      => '?_a=templatemsg&_u=sp.record',
			      'activeurl' => 'sp.record'),
			array('name'  => '微商城通知',
	...
	public function templatelist()
	{
		...
		$template_user_set_list = Template_Msg_WxPlugMod::get_template_msg_set_list($option);
	...
	*/
	public function templateset() {
		...
		if (!($even = requestInt('even')) || empty($template_user_set['public_uid'])) {
			redirectTo('?_a=templatemsg&_u=sp.index');
		}
		if($template_user_set['uid'] = requestInt('uid')) {
			$template_user_set = Template_Msg_WxPlugMod::get_user_template_by_uid($template_user_set['uid']);
			if(!empty($template_user_set['template_data']))
			{
				$template_user_set['template_data'] = Template_Msg_WxPlugMod::get_template_data_back($template_user_set['template_data'],floor($even / 1000));
			}
		}

		$even_args_arr = Template_Msg_WxPlugMod::get_even_args_arr(floor($even / 1000));
		$even_arr = Template_Msg_WxPlugMod::get_even_arr();
		$even_arr          = $even_arr[floor($even / 1000)];
	...
	*/
	public function record() {
		...
		isset($_REQUEST['status']) && $option['status'] = requestInt('status');
 		...
 		$template_msg_record  = Template_Msg_WxPlugMod::get_template_msg_record_list($option);
		$even_arrs = Template_Msg_WxPlugMod::get_even_arr();
		$even_arr =array();
        ...
        $pagination = uct_pagination($option['page'], ceil($template_msg_record['count']/$option['limit']),
            '?_a=templatemsg&_u=sp.record&page=');


        $params = array('template_msg_record'=>$template_msg_record,'even_arr'=>$even_arr,'select_arr'=>$select_arr,'pagination' => $pagination);
		$this->sp_render($params);
	}
