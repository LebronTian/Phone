$(function () {
    ...
    $('.create').click(function(){
        ...
        $.post('?_a=templatexcxmsg&_u=api.get_template_array', data, function(ret){
            ret=$.parseJSON(ret);
            if(ret.errno==0)
            {
    ...
    $('.save').click(function(){
        ...
        $.post('?_a=templatexcxmsg&_u=api.set_user_template_by_tpl_id', data, function(ret){
            ret=$.parseJSON(ret);
            if(ret.errno==0){
                showTip("","保存成功","1000");
                setTimeout(function () {
...
//console.trace();
    $(".makeup-div").each(function (i) {
		...
		console.log('old ', textarea.val(),textarea.text(),  'new ', text);
        textarea.after('<div class="new-input " contenteditable="true">'+text+'</div>').hide();//todo******************hide()
		console.log('after ', i, textarea,'<div class="new-input " contenteditable="true">'+text+'</div>');
    })
    $('.new-input').on('focusout', function() {
        var key = $(this).parent().find("textarea").data("key")
        var text = $(this).html()
        console.log('old input', text)
...
function get_teplate_list_all(a)
{
    ...
    $.getJSON('?_a=templatexcxmsg&_u=api.get_xcx_template_list_all',{'refresh':a},function(ret)
    {
        console.log(ret);
        if(ret.errno==0)
        {
...
$('#id_template_id').on('change', function() {
	if(by_amaze_ui) return by_amaze_ui = 0;
