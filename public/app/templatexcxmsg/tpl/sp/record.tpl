...
<div class="am-u-sm-12">
    <table class="am-table am-table-striped am-table-hover">
        ...
        if(!empty($templatexcx_msg_record)){
            if(empty($templatexcx_msg_record)||($templatexcx_msg_record['count']=="0")) {
                echo '<tr class="am-danger"><td>暂无数据！</td></tr>';
            }
            else {
                $html = '';
                foreach($templatexcx_msg_record['list'] as $t){
                    $html.='
...
<script>
    var templatexcx_msg_record = <?php echo(!empty($templatexcx_msg_record)? json_encode($templatexcx_msg_record):"null")?>;
    console.log(templatexcx_msg_record);
