...
//                }
        ...
        <div class="am-u-sm-6 am-u-end">
            <div id="id_template_preview" >
                ...
                <div id="id_template_preview_url">
                  查看详情<span><i class="am-icon-chevron-right" aria-hidden="true"></i></span>
                </div>
    ...
    <div class="am-g am-margin-top-sm template_arr" style="margin-bottom: 20px">
        ...
        if(!empty($template_user_set['template_data']))
        {
            foreach($template_user_set['template_data'] as $tak=>$ta)
            {
				...
				<div class="am-u-sm-6 am-u-end">
				<textarea class="template_arr_value" data-key="'.$tak.'">'.((empty($ta['value'])?'':$ta['value'])).'</textarea>
				</div>
    ...
    <div class="am-g am-margin-top-sm">
        ...
        <div class="am-u-sm-8 am-u-end">
            <input type="text" id="id_page" placeholder="用户点击模板消息后跳转的路径"
                <?php if (!empty($template_user_set['page']))
                {
                    echo 'value="' . $template_user_set['page'] . '"';
                } ?>>
        </div>
    </div>
