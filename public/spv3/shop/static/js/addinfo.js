...
$(document).ready(function () {
    ...
    * */
    if(!(edit_product=="")){
        console.log("?");
        var product_uid = edit_product.uid?edit_product.uid:edit_product.copy_uid;
        $.getJSON("?_a=shop&_u=api.get_product_extra_info",{product_uid:product_uid}, function (ret) {
            console.log("a",ret);
            if(ret.errno==0){
                var infoHtml = '';
    ...
    $(".extra-content").on("click",".del-extra", function () {
        if(confirm("确定要删除吗？")){
            ...
            if(uid){
                ...
                $.post("?_a=shop&_u=api.delete_product_extra_info",{uid:uid,ukey:name}, function (ret) {
                    console.log(ret)
                })
            }
