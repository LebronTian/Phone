...
function do_delete(uids) {
	...
	$.post('?_a=shop&_u=api.delete_order', data, function(ret){
		...
		ret = $.parseJSON(ret);
		if (!ret.data) {
			showTip('err','删除失败！已付款订单不能删除',3000);
		} else {
			showTip('success','删除成功！',3000);
			window.location.reload();
		}
	});
}
...
/*全选******************************/
$('.qbxf-reverse').click(function(){
    var checked = $(this).prop('checked');
	$('.ccheck').prop('checked', checked);
});

// 批量删除
$(".multiDel-btn").click(function() {
	var uids = getSelectedOrders();
	do_delete(uids);
});
...
function getSelectedOrders() {
    var uids = [];
    $('.ccheck:checked').each(function(){
        uids.push($(this).parent().parent().attr('data-id'));
    });
    console.log("selected order >>>>", uids);
    return uids;
}
...
//下拉组件
$(".zhiclw-llcj h5").on("click", function(){
		$(this).siblings("ul").toggle();
});


var astart_time = laydate.render({
	elem: '#sj1',
	type: 'datetime',
	theme: '#0099CC',
	done: function(value, date, endDate) {
		bend_time.config.min = {
			year: date.year,
			month: date.month - 1,
			date: date.date,
			hours: date.hours,
...
$(".screenForm").submit(function(e){
    ...
    var newUrl = "?_a=shop&_u=sp.orderlist&key_productname=" + target[0].value + "&key_uid=" + target[1].value + "&key_suname=" + target[3].value + "&key_suphone=" + target[4].value;
