...
<div class="am-u-sm-3">
	<p><a href="?_a=sp&_u=index.servicedetail&uid=<?php echo $order['service_uid']; 
if(!empty($order['service']['dir'])) echo '&dir='.$order['service']['dir'];?>">
	<strong><?php echo $order['service']['name'] .' X '.(!empty($order['service']['quantity']) ? $order['service']['quantity'] : 1)?></strong>
	<img style="max-width:140px;max-height:140px;display:block;" src="<?php echo $order['service']['main_img'];?>">
	</a></p>
...
<div class="am-u-sm-3">
	<?php 
		if($order['status'] == SpServiceMod::ORDER_WAIT_USER_PAY) {
			echo '<p><a class="am-btn am-btn-lg am-btn-primary" id="id_go_pay" href="?_a=pay&oid=a'.$order['uid']
...
<script>
//$(function(){
window.location.href= $('#id_go_pay').attr('href');
//});
</script>
<?php } ?>
