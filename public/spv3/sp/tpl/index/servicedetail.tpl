...
else if(in_array($service['is_virtual'], array(SpServiceMod::VIRTUAL_ADD_APP))) {
	$dir = requestString('dir', PATTERN_APP_NAME);
	if(!$dir || !($dir = WeixinPlugMod::get_plugin_by_dir($dir)) || !$dir['price']) {
		echo '<script> console.log("warning in redirectTo! headers already sent!!! use js instead!!!");
window.location.href="?_a=sp&_u=index.plugindetail&dir='.$dir.'";</script>';
		die;
		
	}
	if(1 || SpInviteMod::can_sp_install_plugin($dir['dir'])) {
		echo '<script> console.log("warning in redirectTo! headers already sent!!! use js instead!!!");
window.location.href="?_a=sp&_u=index.plugindetail&dir='.$dir['dir'].'&goinstall=1";</script>';
		die;
	}
	...
	$service['sku_table2'] = array(
		'table' => array('名称' => array($dir['name'])),
		'info' => array('名称:'.$dir['name'] => array('price' => $dir['price'], 
						'ori_price' => $dir['price'], 'quantity' => 99000)),
	);
}
...
?>
		...
		<?php
			...
			echo '<p><button class="am-btn am-btn-warning am-btn-lg install" is_virtual="'.$service['is_virtual'].'" data-uid="'.$service['uid'].'" data-dir="'.$dir['dir'].'">立即开通</button></p>';
			} else {
...
<?php
	$extra_js = array('/spv3/sp/static/js/servicedetail.js');
?>
