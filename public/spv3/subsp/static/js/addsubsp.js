...
$('.save').click(function(){
	...
	if(!$('.limit_box').is(':visible')) {
		...
		access_rule = {'*': 0}; //限制
		$.each($('.parent_box input'), function(){
			var k = $(this).val();
			if($(this).attr('link')) {
				//hehe
				if($(this).attr('link') == '?_a=sp') {
					k = 'sp.index.index';
				}
				else {
					console.log('!!! not suport link ', $(this).attr('link'));
				}
				access_rule[k] = $(this).prop('checked') ? 1 : 0;
			} else {
				access_rule[k+'.*'] = $(this).prop('checked') ? 1 : 0;
			}
			/*
	...
	//access_rule['sp.*'] = 1;
	console.log('hehe', access_rule, JSON.stringify(access_rule));
