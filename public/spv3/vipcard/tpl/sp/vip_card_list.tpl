		...
		else
		{
			...
			foreach ($vip_card_list['list'] as $vip_card_su)
			{
				...
				foreach ($vip_card_su['connent'] as $c)
				{
					...
					{
						$html .= '<img style="max-width:64px;max-height:64px;" src="' . $c['value'] . '">';
					}
					else
					{
						$html .= $c['value'];
				...
				$html .= '<a target="_self" class="am-btn am-btn-success " href="?_a=su&_u=sp.fansdetail&uid=' . $vip_card_su['su_uid'] . '">详情</a></td>';
//				if (!empty($_REQUEST['_d']))
//				{
					$html .= '<td>';
