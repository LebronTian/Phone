...
class WxPayDataBase
{
	...
	**/
	...
	public function MakeSign()
	{
		...
		//签名步骤三：MD5加密
		#Weixin::weixin_log($string.' - sign to ->'.md5($string));
		$string = md5($string);
		//签名步骤四：所有字符转为大写
...
class WxPayResults extends WxPayDataBase
{
	...
	{	
		$obj = new self();
		$obj->FromXml($xml);
		Weixin::weixin_log('wxpay init xml '. var_export($obj->GetValues(), true));
		//微信支付回调通知时没有初始化参数
		$otn = $obj->get('out_trade_no');
		if(!empty($otn)) {
			WxpayMod::setConfigByOid($obj->get('out_trade_no'));
		}
