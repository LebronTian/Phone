...
class GmagickHelper
{
	...
	/*
		输出验证码图片
	*/
	public static function out_verify_code($str) {
		$img = new Gmagick();
		$img->newimage(50, 20, 'white', 'png');
        $draw = new GmagickDraw();
		$draw->setFontSize(18);
		$draw->setFillColor(new GmagickPixel('#0a6999'));
		$draw->annotate(5, 16, $str);
		$draw->line(3,4, 40, 18);
		$draw->line(1,8, 44, 1);
		//$img->annotateimage($draw, 5, 16, 5, $str);
		$img->drawImage($draw);

		$buff = $img->getImageBlob();
		$img->destroy();
