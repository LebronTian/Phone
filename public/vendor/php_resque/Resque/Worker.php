...
class Resque_Worker
{
	...
	*/
	...
	public function work($interval = Resque::DEFAULT_INTERVAL, $blocking = false)
	{
		...
		while(true) {
			//app中好多地方使用这个变量，每次都更新一下, 防止worker长时间运行导致时间不是期望的
			//或者也可以在fork子进程时初始化一下
			$_SERVER['REQUEST_TIME'] = time();

			//echo 'job id is ... '.$this->id;
			if(Resque::redis()->get('worker:'.$this->id.':reboot')) {
				break;
			}
			if(Resque::redis()->get('worker:'.$this->id.':fork')) {
				Resque::redis()->del('worker:'.$this->id.':fork');
				
				$end = (explode(':', $this->id, 3));
				$cmd = Resque::get_cmd_add_worker('"'.end($end).'"');

				$this->logger->log(Psr\Log\LogLevel::NOTICE, '{worker} forking ... {cmd} ', array('worker' => $this->id, 'cmd' => $cmd));
				exec($cmd);
			}
			if(!Resque::redis()->sismember('workers', $this->id)) {
				$this->shutdown = true;
			}
			if($this->shutdown) {
				break;
			...
			if(!$job) {
				//没任务时试试跑一下定时任务, 有定时任务的话就不要再sleep
				if(Resque::do_defered_job($this)) {
					continue;
				}
		...
		if(Resque::redis()->get('worker:'.$this->id.':reboot')) {
			Resque::redis()->del('worker:'.$this->id.':reboot');
			$end = (explode(':', $this->id));
			$cmd = Resque::get_cmd_add_worker('"'.end($end).'"');
			$this->logger->log(Psr\Log\LogLevel::NOTICE, '{worker} rebooting ... {cmd} ', array('worker' => $this->id, 'cmd' => $cmd));
			exec($cmd);
		}
	...
	public function workerPids()
	{
		...
		//lhliu add for mac
		exec('ps -A -o pid,command | grep php', $cmdOutput);
		foreach($cmdOutput as $line) {
			list($pids[],) = explode(' ', trim($line), 2);
		}
		return $pids;
